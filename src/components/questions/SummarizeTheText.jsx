...
export default function SummarizeTheText({
  ...
  useEffect(() => {
    if (stage === "WRITING" && !isSectionExpired) {
      setPhase("writing"); // Next enabled
    } else {
      setPhase("prep"); // Next disabled
    }
    ...
    <div className="space-y-4 md:space-y-6 max-w-full">
      ...
      <div className="relative space-y-3">
        <Textarea
          ...
          className={`min-h-[250px] text-lg p-6 transition-all ${"focus-visible:ring-sky-500"}`}
          value={userInput}
          onChange={handleTextChange}
          disabled={stage === "PREP" || isSectionExpired}
        />

        <div className="flex justify-between items-center px-1">
          <div
            className={`px-4 py-2 rounded-full border text-xs font-bold ${"bg-slate-800 text-white"}`}
          >
            Word count: {currentCount}
          </div>
        </div>
      </div>
    </div>
